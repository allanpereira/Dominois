...
Jogo.prototype.AniEntrada = function(jogador){
    ...
    qtdbulletsatual =0;
    megalocal = {
        x : jogador.pedras[0].sprite.phaserSprite.x - 170,
        y : jogador.pedras[0].sprite.phaserSprite.y - 50
    };

    for(var i = 0; i < jogador.pedras.length; i++) {
        var ps = jogador.pedras[i].sprite.phaserSprite;
        var c = game.add.sprite(ps.x, ps.y, 'dominoback');
        c.nome = jogador.pedras[i].nome;
        backdom.add(c);
       
        this.AniMove(c, {
            x :this.tela.tamanho.largura / 2,
            y:this.tela.tamanho.altura / 2
        }, null, 0);

        this.AniMove(jogador.pedras[i], {
            x : this.tela.tamanho.largura / 2,
            y : this.tela.tamanho.altura / 2
        }, null, 0);
    }
